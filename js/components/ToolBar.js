...
export default class ToolBar extends Component {
  ...
  sortUsers(e){
    ...
    const users = this.props.users.slice(0); //copy array to avoid mutation for performance boost
    const filteredUsers = this.props.filteredUsers.slice(0); //copy array to avoid mutation for performance boost

    const sortedUsers = users.sort(sortingFunc(sortType, this.isDirectionAsc));
    const sortedFilteredUsers = filteredUsers.sort(sortingFunc(sortType, this.isDirectionAsc));
